...
public class IAPManager : MonoBehaviour, IStoreListener
{
    ...
    // ✅ NEW: UI / systems can subscribe to hint changes
    // (count, unlimited)
    public event Action<int, bool> OnHintsChanged;

// ✅ CALL THIS when hints data changes (UI sync)
    public void NotifyHintsChanged()
    {
        OnHintsChanged?.Invoke(GetHintsCount(), HasUnlimitedHints());
    }
    ...
    // ✅ NEW: Raise hint state changed event
    private void RaiseHintsChanged()
    {
        OnHintsChanged?.Invoke(GetHintsCount(), HasUnlimitedHints());
    }
    ...
    public void OnInitialized(IStoreController controller, IExtensionProvider extensions)
    {
        ...
        // ✅ Ensure UI sync when store becomes ready
        RaiseHintsChanged();
    }
    ...
    private void AddHints(int count)
    {
        ...
        {
            Debug.Log("[IAPManager] Unlimited hints active - skipping AddHints count change.");
            RaiseHintsChanged();
            return;
        }
    ...
    {
        // Keep existing behavior for "unlimited" return value
        if (HasUnlimitedHints())
        {
