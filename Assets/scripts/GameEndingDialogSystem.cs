...
public class GameEndingDialogSystem : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Debug.Log("[GameEndingDialogSystem] 🔧 DEBUG KEY PRESSED - Starting ending dialog!");
            StartEndingDialog();
        }
        #endif
    }
    ...
    private void ShowCurrentDialog()
    {
        ...
        if (currentDialog < dialogBubbles.Length && dialogBubbles[currentDialog] != null)
        {
            ...
            Debug.Log($"[GameEndingDialogSystem] ✅ Bubble {currentDialog} is now popping in! Previous bubbles stay visible.");
        }
        else
        {
    ...
    private void TriggerLevelEndAnimators()
    {
        ...
        Debug.Log($"[GameEndingDialogSystem] 🎬 Playing {imageAnimations.Length} level-end animations");
        ...
        for (int i = 0; i < imageAnimations.Length; i++)
        {
            ...
            if (hasClipName)
            {
                ...
                anim.Play(clipName);
                Debug.Log($"[GameEndingDialogSystem] 🎬 Playing clip '{clipName}' on '{anim.gameObject.name}'");
            }
            else
            {
                // Play default animation
                anim.Play();
                Debug.Log($"[GameEndingDialogSystem] 🎬 Playing default clip on '{anim.gameObject.name}'");
            }
        }
    }
    ...
    private IEnumerator AnimateBubblePopIn(GameObject bubble)
    {
        ...
        Debug.Log($"[GameEndingDialogSystem] 💥 Bubble {currentDialog} pop animation complete!");
    }
    ...
    private void PlayBubbleSound()
    {
        ...
        Debug.Log($"[GameEndingDialogSystem] 🔊 Playing bubble pop sound {currentDialog}");
    }
    ...
    public void StartEndingDialog()
    {
        ...
        Debug.Log($"[GameEndingDialogSystem] After StartEndingDialog - auto advance: {autoAdvance}");
    }
    ...
    {
        Debug.Log("[GameEndingDialogSystem] 🎬 Starting auto-advance sequence");
        ...
        for (int i = 0; i < dialogBubbles.Length; i++)
        {
            ...
            Debug.Log($"[GameEndingDialogSystem] Showing bubble {i}/{dialogBubbles.Length - 1}");

            // ✅ המתן 0.3 שניות לפני הבועה הבאה (כל הבועות נשארות על המסך!)
            yield return new WaitForSeconds(delayBetweenBubbles);
        }

        Debug.Log("[GameEndingDialogSystem] ✅ All bubbles shown! Displaying all together...");

        // ✅ כל הבועות על המסך - המתן את הזמן שהן נשארות ביחד
        yield return new WaitForSeconds(allBubblesDisplayTime);

        Debug.Log("[GameEndingDialogSystem] Starting end game sequence...");
