...
public class LoadingManager : MonoBehaviour
{
    ...
    [Tooltip("SFX will start when loading reaches this percentage")]
    [Range(0f, 100f)]
    [SerializeField] private float sfxStartPercent = 10f;
    ...
    private void Start()
    {
        // DON'T play music here - it will start at sfxStartPercent instead
    ...
    private IEnumerator LoadGame()
    {
        ...
        while (elapsed < loadingDuration)
        {
            ...
            // ✅ Start music when reaching sfxStartPercent (default 10%)
            if (!musicStarted && percent >= sfxStartPercent)
            {
                if (audioSource != null && loadingMusic != null)
                {
                    ...
                    audioSource.Play();
                    musicStarted = true;
                    Debug.Log($"[LoadingManager] 🎵 Music started at {percent:F1}%");
                }
            }
